...
public static class CarpetasEndpoints
{
    public static void MapearCarpetasEndpoints(this WebApplication app)
    {
        ...
        grupo.MapPost("/", async (CrearCarpetaRequest solicitud, ClaimsPrincipal usuario, ICarpetasServicio servicio) =>
        {
            ...
            var usuarioId = ObtenerUsuarioId(usuario);
            var carpeta = await servicio.Crear(solicitud.Nombre, usuarioId);
        ...
        grupo.MapDelete("/{id:guid}", async (Guid id, ClaimsPrincipal usuario, ICarpetasServicio servicio) =>
        {
            var usuarioId = ObtenerUsuarioId(usuario);
            var eliminada = await servicio.Eliminar(id, usuarioId);
            if (!eliminada)
                return Results.NotFound(new Respuesta<object>(
                    Exito: false, Mensaje: "Carpeta no encontrada", Data: null));
            return Results.Ok(new Respuesta<object>(
                Exito: true, Mensaje: "Carpeta eliminada", Data: null));
        }).WithSummary("Elimina una carpeta");
